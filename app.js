...
function renderProducts(filter='all'){const list=products.filter(p=>filter==='all'||p.category===filter);$('#products').innerHTML=list.map(p=>{const i=products.indexOf(p);return `<article class="product"><button class="product-visual tone-${i}" data-detail="${p.id}" aria-label="View ${p.name}"><span>${p.category==='Fat loss'?'Weight management':p.category}</span></button><div class="product-body"><h3 class="product-name">${p.name}</h3><div class="approval-status">${p.status}</div><p>${p.desc}</p><div class="product-bottom"><div><span class="price">${money(p.price)}<small>illustrative price</small></span><button class="text-button" data-detail="${p.id}">View profile →</button></div><button class="add" data-add="${p.id}" aria-label="Add ${p.name} to demo bag">+</button></div></div></article>`}).join('');$('#result-count').textContent=`${list.length} profiles`}
...
function detail(id){const p=products.find(x=>x.id===id);if(!p)return;const i=products.indexOf(p);$('#product-detail').innerHTML=`<p class="eyebrow">PEPTIDE PROFILE / ${String(i+1).padStart(2,'0')}</p><h2 id="detail-title">${p.name}</h2><div class="detail-image tone-${i}" role="img" aria-label="Concept vial photograph"></div><div class="approval-status">${p.status}</div><p>${p.detail}</p>${p.source?`<p><a href="${p.source}" target="_blank" rel="noopener">Read the source ↗</a></p>`:''}<p class="notice">Demo listing only. No injectable product is available. ${money(p.price)} is a sample price, not a market quote.</p><button class="button" data-add="${p.id}">Add to demo bag — ${money(p.price)}</button>`;$('#product-dialog').showModal()}
...
document.querySelectorAll('a[data-category]').forEach(link=>link.addEventListener('click',()=>{const filter=document.querySelector(`[data-filter="${link.dataset.category}"]`);filter?.click()}));
